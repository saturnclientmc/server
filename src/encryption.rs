...
pub fn handshake(mut stream: TcpStream, rsa: Rsa<Private>) -> Result<ETcp, Error> {
    ...
    reader.read_line(&mut aes_encoded)
    ...
    let aes_len = rsa.private_decrypt(
        &encrypted_data,
        &mut aes_decrypted,
...
impl ETcp {
    ...
    pub fn read(&mut self) -> Result<Option<String>, Error> {
        ...
        if self.reader.read_line(&mut v_encoded)
            .map_err(|e| Error::NetworkError(format!("Failed to read line: {}", e)))? == 0 {
                return Ok(None);
            }
    ...
    pub fn clone(&mut self) -> Result<Self, Error> {
